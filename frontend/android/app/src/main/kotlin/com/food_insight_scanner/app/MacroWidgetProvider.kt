        ...
        try {
            for (appWidgetId in appWidgetIds) {
                val views = RemoteViews(context.packageName, R.layout.widget_layout).apply {
                    // Fetch data saved from Flutter safely
                    val calGoal = getSafeInt(widgetData, "calories_goal", 2500)
                    val calConsumed = getSafeInt(widgetData, "calories_consumed", 0)
                    val carbsGoal = getSafeInt(widgetData, "carbs_goal", 300)
                    val carbsConsumed = getSafeInt(widgetData, "carbs_consumed", 0)
                    val proteinGoal = getSafeInt(widgetData, "protein_goal", 150)
                    val proteinConsumed = getSafeInt(widgetData, "protein_consumed", 0)
                    val fatGoal = getSafeInt(widgetData, "fat_goal", 80)
                    val fatConsumed = getSafeInt(widgetData, "fat_consumed", 0)
            ...
            Log.e("MacroWidgetProvider", "Widget update error: ${e.message}", e)
        }
    }
